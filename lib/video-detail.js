...
module.exports = async (dir_name, videoId, config = require('config')) => {
  const { apiKey, base_url, retryOpt, requestOpt, saveData } = config.videoDetail
  const url = base_url + '&key=' + apiKey + '&id=' + videoId
  var newDate = new Date()
  var time = newDate.toFormat('YYYY-MM-DDTHH24:MI:SS.000Z')
  const fileName = dir_name + videoId + '-' + time + '_videoDetail' + '.txt'
